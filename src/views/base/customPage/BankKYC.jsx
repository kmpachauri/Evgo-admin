  ...
  const getUserLists = async () => {
    try {
      const data = await fetchData({
        url: '/api/v1/admin/user/getuser-bankKyc',
        method: 'GET',
      })
  ...
  const handleSearch = () => {
    ...
    if (filters.search) {
      filteredList = filteredList.filter(
        (user) =>
          user.user_id?.userId?.toLowerCase().includes(filters.search.toLowerCase().trim()) ||
          user.user_id?.name?.toLowerCase().includes(filters.search.toLowerCase().trim())
      )
    }
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetchData({
            url: `/api/v1/admin/user/user-bankKyc/${id}`,
            method: 'PUT',
            ...
            <CTable striped bordered hover responsive>
              <CTableHead color="primary">
                <CTableRow>
                  ...
                  <CTableHeaderCell>PanCard</CTableHeaderCell>
              ...
              <CTableBody>
                {paginatedUsers.map((user, index) => (
                  <CTableRow key={user._id}>
                    ...
                    <CTableDataCell>{user?.ifscCode || 'N/A'}</CTableDataCell>
                    <CTableDataCell>{user?.panNumber || 'N/A'}</CTableDataCell>
                    <CTableDataCell>{user?.bankName || 'N/A'}</CTableDataCell>
                    <CTableDataCell>{user?.branchName || 'N/A'}</CTableDataCell>
                    <CTableDataCell>{user?.status || 'N/A'}</CTableDataCell>
                       <CTableDataCell>
                                          {user.pancardPhoto ? (
                                            <img
                                            src={`${imgBaseUrl}${normalizePath(user.pancardPhoto)}`}
                                            alt="Passbook"
                                            style={{ width: '180px', height: '80px', objectFit: 'contain', borderRadius: '8px', cursor: 'pointer' }}
                                            onClick={() => {
                                              setSelectedImage(`${imgBaseUrl}${normalizePath(user.pancardPhoto)}`);
                                        ...
                                        <CTableDataCell>
                                          {user.passbookPhoto ? (
                                            <img
                                            src={`${imgBaseUrl}${normalizePath(user.passbookPhoto)}`}
                                            alt="Passbook"
                                            style={{ width: '180px', height: '80px', objectFit: 'contain', borderRadius: '8px', cursor: 'pointer' }}
                                            onClick={() => {
                                              setSelectedImage(`${imgBaseUrl}${normalizePath(user.passbookPhoto)}`);
                                              setScreenshotModal(true);
                                            }}
                                            />
                                          ) : (
